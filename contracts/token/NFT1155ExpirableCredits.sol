...
contract NFT1155ExpirableCredits is NFT1155Base {
     ...
     * @dev Records the burn against non-expired mint entries using the plan-clamped
     * `_creditsToRedeem`, so the expirable ledger decrements by the same amount ERC1155
     * `_balances` does. Running *after* `_canRedeemCredits` ensures the permission check
     * sees the pre-burn balance — required for `ONLY_SUBSCRIBER` plans to allow burning
     * more than half of remaining credits in a single call (issue #170).
     ...
     * @dev Records a burn entry per plan using the plan-clamped per-item amounts, keeping
     * the expirable ledger aligned with what `_burnBatch` will consume (issue #173).
     * @param _from Address whose credits will be burned
