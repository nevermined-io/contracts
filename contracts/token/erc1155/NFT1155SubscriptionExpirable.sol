...
contract NFT1155SubscriptionExpirable is NFT1155Upgradeable {
     ...
     * 
     */
    function mint(address to, uint256 tokenId, uint256 amount, uint256 expirationSecs, bytes memory data) public virtual {
        super.mint(to, tokenId, amount, data);
        bytes32 _key = _getTokenKey(to, tokenId);

        _tokens[_key].push( MintedTokens(amount, expirationSecs, block.timestamp, true));
    }
...
//            bytes32(id), _msgSender(), keccak256('burn'), '', 'burn');
        ...
        for (uint index = 0; index < _tokens[_key].length; index++) {
            ...
            if (entry.expirationSecs == 0 || block.timestamp < (entry.mintTimestamp + entry.expirationSecs))   {
                if (_pendingToBurn <= entry.amountMinted) {
                    _tokens[_key].push( MintedTokens(_pendingToBurn, entry.expirationSecs, block.timestamp, false));
                    break;
                } else {
                    _pendingToBurn -= entry.amountMinted;
                    _tokens[_key].push( MintedTokens(entry.amountMinted, entry.expirationSecs, block.timestamp, false));
                }   
            }
        }
    }    
    ...
    function balanceOf(address account, uint256 tokenId) public view virtual override returns (uint256) {
        ...
        for (uint index = 0; index < _tokens[_key].length; index++) {
            if (_tokens[_key][index].mintTimestamp > 0 &&
                (_tokens[_key][index].expirationSecs == 0 || block.timestamp < (_tokens[_key][index].mintTimestamp + _tokens[_key][index].expirationSecs))) {
    ...
    function whenWasMinted(address owner, uint256 tokenId) public view returns (uint256[] memory) {
        ...
        for (uint index = 0; index < _tokens[_key].length; index++) {
            _whenMinted[index] = _tokens[_key][index].mintTimestamp;
        }
        return _whenMinted;
    }
