...
contract('NFT721LockCondition', (accounts) => {
    ...
    describe('fulfill correctly', () => {
        it('should fulfill if conditions exist for account address', async () => {
            ...
            const { tokenIds, eventIds } = await erc721.tokenDetailsOfOwner(owner)
...
//            await erc721.approve(lockCondition.address, tokenId, { from: owner})
            await erc721.setApprovalForAll(lockCondition.address, true)

            const hashValues = await lockCondition.hashValues(did, lockAddress, tokenId, nftTokenAddress)
            ...
            const result = await lockCondition.fulfill(agreementId, did, lockAddress, tokenId, nftTokenAddress, { from: owner } )
    ...
    describe('trying to fulfill but is invalid', () => {
        it('should not fulfill if conditions do not exist', async () => {
            ...
            await assert.isRejected(
                lockCondition.fulfill(agreementId, did, lockAddress, tokenId, nftTokenAddress),
        ...
        it('out of balance should fail to fulfill', async () => {
            ...
            await assert.isRejected(
                lockCondition.fulfill(agreementId, did, lockAddress, tokenId, nftTokenAddress, { from: accounts[2] }),
                undefined
            )
        })
        ...
        it('right transfer should fail to fulfill if conditions already fulfilled', async () => {
            ...
            await lockCondition.fulfill(agreementId, did, lockAddress, tokenId, nftTokenAddress)
            assert.strictEqual(
